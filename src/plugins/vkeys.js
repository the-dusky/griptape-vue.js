    ...
    Vue.prototype.$store.registerModule('$vkeys', {
      ...
      getters: {
        listViewingKeys: (state) => {
          return (userAddress) => {
            const filteredVkeys = state.vkeys.filter(vkey => vkey.userAddress == userAddress);
            return (filteredVkeys) ? filteredVkeys[0].viewingKeys : '';
          }
        },
        getViewingKey: (state) => {
          return (userAddress, contractAddress) => {
            const filteredData = state.vkeys.filter(vkey => vkey.userAddress == userAddress);
            const keys = filteredData.filter(viewingKey => viewingKey.contractAddress == contractAddress);
            return (keys) ? keys[0] : '';
          }
        },
      },
